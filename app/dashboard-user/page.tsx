...
export default function DashboardUserPage() {
  ...
  const [textJustificatif, setTextJustificatif] = useState<string>("");
  ...
  // Gestion du texte pour les heures sup
  const handleTextChange = (id: number, value: string) => {
    setEditingId(id);
    setTextJustificatif(value);
  };

  const handleTextUpdate = async (d: any) => {
    try {
      const res = await fetch(`/api/demande/${d.id_demande}/justificatif`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ justificatif: textJustificatif }),
        credentials: "include",
      });
      const data = await res.json();
      if (data.success) {
        setDemandes(prev =>
          prev.map(dem =>
            dem.id_demande === d.id_demande
              ? { ...dem, justificatif: textJustificatif }
              : dem
          )
        );
        setEditingId(null);
        ...
        alert(data.error || "Erreur lors de la mise à jour du justificatif");
      }
    } catch (err) {
      console.error(err);
  ...
  // Gestion du fichier pour maladie ou autres
  ...
  const handleUpload = async (id: number) => {
    ...
    try {
      const res = await fetch("/api/demande/justificatif", {
        method: "PATCH",
        body: formData,
        credentials: "include",
      });
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-white pr-[200px] pl-[200px]">
      ...
      <div className="flex w-full flex-col justify-center items-center bg-[#000091] text-white p-6 mt-6 space-y-4 overflow-y-scroll rounded-2xl shadow-lg">
        ...
        {demandes.map(d => (
          <div key={d.id_demande} className="w-full flex flex-row justify-between items-center bg-[#1a1a9c] p-4 rounded-lg shadow-md">

            {/* Infos utilisateur */}
            <div className="flex items-center space-x-4 w-1/5">
              <img src={d.photo || "/uploads/default.png"} alt="Avatar" className="w-10 h-10 rounded-full border border-white" />
            ...
            <div className="flex flex-col w-1/6 items-center">
              ...
              {/* Heures sup */}
              {d.type === "hsup" && d.statut_demande === "en attente" && (
                <>
                  <Input
                    type="text"
                    value={editingId === d.id_demande ? textJustificatif : d.justificatif || ""}
                    onChange={(e) => handleTextChange(d.id_demande, e.target.value)}
                    className="text-black mb-2 bg-indigo-100"
                  />
                  <Button
                    onClick={() => handleTextUpdate(d)}
                    disabled={editingId !== d.id_demande}
                    className="bg-green-500 text-white hover:bg-green-600"
                  >
                    {editingId === d.id_demande ? "Mettre à jour..." : "Mettre à jour"}
                  </Button>
                </>
              )}

              {d.type !== "hsup" && d.justificatif && d.justificatif !== "Justificatif non nécessaire" && (
                <>
                  <a
                    href={d.justificatif}
                    download
